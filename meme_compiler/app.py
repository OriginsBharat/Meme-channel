...
class MemeCompilerApp(tk.Tk):
    ...
    def _create_compiler_tab(self, parent):
        ...
        self.video_preview_label = ttk.Label(self.preview_frame)
    ...
    def _create_settings_tab(self, parent):
        ...
        ttk.Button(parent, text="Preview Voice", command=self._preview_voice).grid(row=4, column=2, padx=5, pady=10)
        ttk.Label(parent, textvariable=self.char_count_var).grid(row=4, column=0, padx=5, pady=10, sticky='w')
        ttk.Button(parent, text="Refresh Voices & Char Count", command=self._refresh_elevenlabs_data).grid(row=5, column=1, pady=10, sticky='w')
        ...
        ttk.Button(parent, text="Save Settings", command=lambda: self.save_config()).grid(row=7, column=1, pady=20, sticky='e')
    ...
    def _browse_tesseract(self):
        ...
        if path: self.tesseract_cmd_path.set(path)

    def _browse_client_secrets(self):
        path = filedialog.askopenfilename(title="Select YouTube client_secrets.json", filetypes=[("JSON files", "*.json")])
        if path: self.youtube_client_secrets_path.set(path)

    def _preview_voice(self):
        api_key, voice_id = self.elevenlabs_api_key.get(), self.selected_voice_id.get()
    ...
    def _refresh_elevenlabs_data(self):
        ...
        if sub_info: self.char_count_var.set(f"Characters Left: {sub_info.character_limit - sub_info.character_count}/{sub_info.character_limit}")
        else: self.char_count_var.set("Characters Left: Error")
    ...
    def _load_image_preview(self, url):
        ...
        w, h = self.preview_frame.winfo_width() - 10, self.preview_frame.winfo_height() - 10
        ...
        def play_video():
            try:
                if self.video_preview_label.winfo_ismapped():
                    self.video_player = tkvideo(video_path, self.video_preview_label, loop=1, size=(w, h))
    ...
    def compile_video(self):
        ...
        args = {"selected_memes": selected_memes, "intro_path": self.intro_full_path, "outro_path": self.outro_full_path, "background_path": self.background_full_path, "output_path": temp_video_path, "enable_tts": self.tts_enabled_var.get(), "elevenlabs_api_key": self.elevenlabs_api_key.get(), "tesseract_cmd_path": self.tesseract_cmd_path.get(), "voice_id": self.voices_map.get(self.selected_voice_id.get()), "vertical_format": self.vertical_format_var.get(), "music_path": self.music_full_path}
    ...
    def save_config(self):
        config_data = {"reddit_client_id": self.reddit_client_id.get(), "reddit_client_secret": self.reddit_client_secret.get(), "tesseract_cmd_path": self.tesseract_cmd_path.get(), "elevenlabs_api_key": self.elevenlabs_api_key.get(), "selected_voice_id_name": self.selected_voice_id.get(), "tts_enabled": self.tts_enabled_var.get(), "vertical_format": self.vertical_format_var.get(), "youtube_client_secrets_path": self.youtube_client_secrets_path.get()}
        try:
    ...
    def load_config(self):
        try:
            ...
            if self.elevenlabs_api_key.get():
                ...
                if saved_voice_name in self.voices_map: self.selected_voice_id.set(saved_voice_name)
        except FileNotFoundError: print("Config file not found. Starting with default settings.")
    ...
    def on_closing(self):
        ...
        if os.path.exists(TEMP_PREVIEW_DIR):
            try: shutil.rmtree(TEMP_PREVIEW_DIR), print(f"Removed temporary preview directory: {TEMP_PREVIEW_DIR}")
            except Exception as e: print(f"Error removing temp directory: {e}")
        self.save_config()
        self.destroy()
...
class FinalPreviewWindow(tk.Toplevel):
    def __init__(self, master, video_path, tts_failures):
        ...
        self.master, self.video_path, self.video_player = master, video_path, None
        self.title("Final Preview & Upload"), self.geometry("960x720"), self.configure(bg=BG_COLOR)
        self.grab_set(), self.focus_set(), self.transient(master)
    ...
    def play_video(self):
        try:
            self.video_player = tkvideo(self.video_path, self.video_label, loop=1, size=(940, 528))
            self.video_player.play()
        except Exception as e:
            self.video_label.config(text=f"Error playing video preview: {e}"), print(f"Error playing final preview: {e}")
    ...
    def save_a_copy(self):
        save_path = filedialog.asksaveasfilename(initialfile=os.path.basename(self.video_path), defaultextension=".mp4", filetypes=[("MP4 Video", "*.mp4")])
        if save_path:
            try: shutil.copy(self.video_path, save_path), messagebox.showinfo("Success", f"Video saved successfully to {save_path}")
            except Exception as e: messagebox.showerror("Save Error", f"Failed to save video: {e}")
    ...
    def cleanup_and_close(self):
        ...
        if os.path.exists(self.video_path):
            try: os.remove(self.video_path), print(f"Removed temporary video file: {self.video_path}")
            except Exception as e: print(f"Error removing temporary video file: {e}")
        self.destroy()

class UploadDialog(tk.Toplevel):
    def __init__(self, master, video_path, client_secrets_path):
        super().__init__(master)
        self.master, self.video_path, self.client_secrets_path = master, video_path, client_secrets_path
        self.title("YouTube Upload Details"), self.geometry("500x400"), self.configure(bg=BG_COLOR)
        self.grab_set(), self.focus_set(), self.transient(master)
        ...
        ttk.Combobox(self, textvariable=self.privacy_var, values=["public", "private", "unlisted"], state="readonly").pack()
    ...
    def start_upload(self):
        ...
        upload_details = {"file_path": self.video_path, "title": self.title_var.get(), "description": self.desc_text.get("1.0", tk.END), "privacy_status": self.privacy_var.get()}
    ...
    def _upload_thread(self, details):
        ...
        if not service:
            self.after(0, self.on_upload_complete, False, "Failed to authenticate with YouTube.", None)
            return
        video_id = youtube_uploader.upload_video(youtube_service=service, file_path=details["file_path"], title=details["title"], description=details["description"], privacy_status=details["privacy_status"])
