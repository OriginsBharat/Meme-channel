...
def _resize_and_crop_to_fill(clip, target_size):
    ...
    return vfx.crop(resized_clip, width=target_size[0], height=target_size[1], x_center=resized_clip.w / 2, y_center=resized_clip.h / 2)
    ...
    try:
        ...
        if music_path:
            ...
            if composited_segment.audio:
                composited_segment.audio = CompositeAudioClip([composited_segment.audio, background_music])
            else:
